...
@objc protocol NBViewControllerDelegate: class {
...
class NBViewController: UIViewController, NISessionDelegate {


    // Delegate
    var delegate: NBViewControllerDelegate?
   
    // MARK: - `IBOutlet` instances.
    @IBOutlet weak var maskHappyFaceLabel: UILabel!
    ...
    let nearbyDistanceThreshold: Float = 1.8288
    var priorDistance: Float = 0.0
    var smoothDistance: Float = 0.0
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.maskHappyFaceLabel.alpha = 0.0
        maskHappyFaceLabel.text = "😀"
    ...
    func getDistanceDirectionState(from nearbyObject: NINearbyObject) -> DistanceDirectionState {
        ...
        // ignore direction for this demo
    ...
    private func animate(from currentState: DistanceDirectionState, to nextState: DistanceDirectionState, with peer: NINearbyObject) {
       ...
       //      detailDistanceLabel.text = String(format: "%0.1f m", peer.distance!) + String(format: " %0.1f f", metersToFeet(meters: peer.distance!))
            
   
 
            detailDistanceLabel.text = String(format: "%0.1f m", smoothTheDistance(peer: peer)) + String(format: " %0.1f f", metersToFeet(meters: smoothTheDistance(peer: peer)))

            
        }
       
     //   this moves the label left and right or right and left
    ...
    func smoothTheDistance(peer: NINearbyObject) -> Float
    {
        
       
        if (abs(peer.distance! - priorDistance)) < 0.1   {

            smoothDistance = priorDistance
        }
        else
        {
        //    print (abs((peer.distance! - priorDistance)), peer.distance!, " " , smoothDistance, " ", priorDistance)
            smoothDistance = peer.distance!
            priorDistance = peer.distance!
        }
