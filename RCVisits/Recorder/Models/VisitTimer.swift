...
class VisitTimer: ObservableObject {
     ...
     */
    init(lengthInMinutes: Int = 0, attendees: [DailyVisit.Attendee] = []) {
     ...
     */
    func reset(lengthInMinutes: Int, attendees: [DailyVisit.Attendee]) {
        self.lengthInMinutes = lengthInMinutes
        self.speakers = attendees.speakers
        secondsRemaining = lengthInSeconds
        activeSpeaker = speakerText
...
extension DailyVisit {
    ...
    var timer: VisitTimer {
        VisitTimer(lengthInMinutes: lengthInMinutes, attendees: attendees)
    }
}

extension Array where Element == DailyVisit.Attendee {
    var speakers: [VisitTimer.Speaker] {
        if isEmpty {
            return [VisitTimer.Speaker(name: "Speaker 1", isCompleted: false)]
        } else {
            return map { VisitTimer.Speaker(name: $0.name, isCompleted: false) }
        }
    }
}
